        ...
        with open(FLAGS.train_path,encoding='cp1252') as f, \
         open(FLAGS.train_svm_path,encoding='cp1252') as svm:
            ...
            for train_idx, val_idx in kf.split(words, sent):
                ...
                with open(str(FLAGS.hardcoded_path) + "/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/cross_train_'+ str(i) +'.txt', 'w') as train, \
                open(str(FLAGS.hardcoded_path) + "/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/cross_val_'+ str(i) +'.txt', 'w') as val, \
                open(str(FLAGS.hardcoded_path) + "/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/svm/cross_train_svm_'+ str(i) +'.txt', 'w') as svmtrain, \
                open(str(FLAGS.hardcoded_path) + "/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/svm/cross_val_svm_'+ str(i) +'.txt', 'w') as svmval:
                    for row in words_1:
                        train.write(row[0])
                        train.write(row[1])
                        train.write(row[2])
                ...
                i += 1
        #get statistic properties from txt file
    train_size, train_polarity_vector = getStatsFromFile(str(FLAGS.hardcoded_path) + "data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/cross_train_0.txt')
    test_size, test_polarity_vector = [], []
    for i in range(split_size):
        test_size_i, test_polarity_vector_i = getStatsFromFile(str(FLAGS.hardcoded_path) + "data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/cross_val_'+str(i)+'.txt')
