...
def softmax_with_len(inputs, length, max_len):
    ...
    if length is not None:
        ...
        mask = tf.reshape(tf.cast(tf.sequence_mask(length, max_len), tf.float32), tf.shape(input=inputs))
        inputs *= mask
    _sum = tf.reduce_sum(input_tensor=inputs, axis=-1, keepdims=True) + 1e-9
    return inputs / _sum
...
def bilinear_attention_layer(inputs, attend, length, n_hidden, l2_reg, random_base, scope_name, layer_id=1):
    ...
    max_len = tf.shape(input=inputs)[1]
    ...
    with tf.compat.v1.variable_scope(scope_name, reuse=tf.compat.v1.AUTO_REUSE) as scope:
        ...
        b = tf.compat.v1.get_variable(
            ...
            initializer=tf.compat.v1.random_uniform_initializer(-0., 0.),
            regularizer=tf.keras.regularizers.l2(0.5 * (l2_reg))
        )
...
def dot_produce_attention_layer(inputs, length, n_hidden, l2_reg, random_base, scope_name, layer_id=1):
    ...
    with tf.compat.v1.variable_scope(scope_name, reuse=tf.compat.v1.AUTO_REUSE) as scope:
        u = tf.compat.v1.get_variable(
            ...
            shape=[n_hidden, 1],
            initializer=tf.compat.v1.random_normal_initializer(mean=0., stddev=np.sqrt(2. / (n_hidden + 1))),
...
def Mlp_attention_layer(inputs, length, n_hidden, l2_reg, random_base, layer_id=1):
    ...
    w = tf.compat.v1.get_variable(
        name='att_w_' + str(layer_id),
        shape=[n_hidden, n_hidden],
        initializer=tf.compat.v1.random_normal_initializer(mean=0., stddev=np.sqrt(2. / (n_hidden + n_hidden))),
        # initializer=tf.random_uniform_initializer(-random_base, random_base),
        # initializer=tf.random_uniform_initializer(-np.sqrt(6.0 / (n_hidden + n_hidden)), np.sqrt(6.0 / (n_hidden + n_hidden))),
        regularizer=tf.keras.regularizers.l2(0.5 * (l2_reg))
    )
    ...
    inputs = tf.transpose(a=tf.reshape(inputs, [-1, n_hidden]), perm=[1, 0])
    tmp = tf.transpose(a=tf.tanh(tf.matmul(w, inputs)), perm=[1, 0])
    tmp = tf.reshape(tf.matmul(tmp, u), [batch_size, 1, max_len])
    alpha = softmax_with_len(tmp, length, max_len)
    return alpha
...
def triple_attention_layer(input1, input2, input3, length, n_hidden, l2_reg, random_base, layer_id=1):
    ...
    max_len = tf.shape(input=input1)[1]
...
def mlp_layer_woalpha(inputs, n_hidden, l2_reg, random_base, layer_id=1):
    ...
    w = tf.compat.v1.get_variable(
        ...
        shape=[n_hidden, n_hidden],
        # initializer=tf.random_normal_initializer(mean=0., stddev=np.sqrt(2. / (n_hidden + n_hidden))),
        zorg=tf.compat.v1.random_uniform_initializer(-random_base, random_base),
        # initializer=tf.random_uniform_initializer(-np.sqrt(6.0 / (n_hidden + n_hidden)), np.sqrt(6.0 / (n_hidden + n_hidden))),
        regularizer=tf.keras.regularizers.l2(0.5 * (l2_reg))
    )
    b = tf.compat.v1.get_variable(
