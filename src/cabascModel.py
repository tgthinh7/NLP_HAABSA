...
def cabasc(input_fw, input_bw, sen_len_fw, sen_len_bw, target, sen_len_tr, ave_sent, mult_mask, sent_full, keep_prob1, keep_prob2, _id='all'):
    ...
    beta_left = tf.squeeze(tf.nn.dropout(beta_left, rate=1 - (keep_prob1))) # batch_size x N
    ...
    beta_right =  tf.squeeze(tf.nn.dropout(beta_right, rate=1 - (keep_prob1))) # batch_size x N
    ...
    v_ns = tf.nn.dropout(v_ns, rate=1 - (keep_prob1))
...
def main(train_path, test_path, accuracyOnt, test_size, remaining_size, learning_rate=FLAGS.learning_rate, keep_prob = FLAGS.keep_prob1):
    ...
    with tf.device('/gpu:1'):
        ...
        with tf.compat.v1.name_scope('inputs'):
            x = tf.compat.v1.placeholder(tf.int32, [None, FLAGS.max_sentence_len], name="x")
            y = tf.compat.v1.placeholder(tf.float32, [None, FLAGS.n_class], name="y")
            sen_len = tf.compat.v1.placeholder(tf.int32, None, name="sen_len")

            x_bw = tf.compat.v1.placeholder(tf.int32, [None, FLAGS.max_sentence_len], name="x_bw")
            sen_len_bw = tf.compat.v1.placeholder(tf.int32, [None], name="sen_len_bw")

            target_words = tf.compat.v1.placeholder(tf.int32, [None, FLAGS.max_target_len], name="target_words")
            tar_len = tf.compat.v1.placeholder(tf.int32, [None], name="tar_len")

            sent_short = tf.compat.v1.placeholder(tf.int32, [None, None], name="sent_short")
            sent = tf.compat.v1.placeholder(tf.int32, [None, FLAGS.max_sentence_len], name="sent")
            mult_mask = tf.compat.v1.placeholder(tf.float32, [None, FLAGS.max_sentence_len], name="mult_mask")

        inputs_fw = tf.nn.embedding_lookup(params=word_embedding, ids=x)       # batch x N x d
        inputs_bw = tf.nn.embedding_lookup(params=word_embedding, ids=x_bw)    # batch x N x d
        sent_full = tf.nn.embedding_lookup(params=word_embedding, ids=sent)    # batch x N x d
        ...
        sentence = tf.nn.embedding_lookup(params=word_embedding, ids=tf.multiply(sent_short,sentence_mask))
        ave_sent = tf.divide(tf.reduce_sum(input_tensor=sentence, axis=1),tf.reshape(tf.tile(tf.cast(sen_len_bw + sen_len - tar_len, tf.float32),[FLAGS.embedding_dim]),[-1,FLAGS.embedding_dim]))
        
        target_mask = tf.cast(tf.sequence_mask(tar_len, FLAGS.max_target_len), tf.int32)
        target = tf.nn.embedding_lookup(params=word_embedding, ids=tf.multiply(target_words,target_mask))
        target = tf.divide(tf.reduce_sum(input_tensor=target, axis=1),tf.reshape(tf.tile(tf.cast(tar_len, tf.float32), [FLAGS.embedding_dim]),[-1,FLAGS.embedding_dim]))
        ...
        optimizer = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=learning_rate).minimize(loss, global_step=global_step)
