...
nltk.internals.config_java('C:/Program Files (x86)/Java/jre1.8.0_351/bin/')
java_path = 'C:/Program Files (x86)/Java/jre1.8.0_351/bin/'
os.environ['JAVAHOME'] = java_path
#owlready2.JAVA_EXE = '/System/Library/Frameworks/JavaVM.framework/Versions/Current/Commands/java'

class OntReasoner():
    def __init__(self):
        onto_path.append(str(FLAGS.hardcoded_path) + "/data/externalData")  # Path to ontology
    ...
    def is_negated(self, word, words_in_sentence):
        ...
        path_to_jar = str(FLAGS.hardcoded_path) + 'data/externalData/stanford-parser-full-2018-02-27/stanford-parser.jar'
        path_to_models_jar = str(FLAGS.hardcoded_path) + 'data/externalData/stanford-parser-full-2018-02-27/stanford-parser-3.9.1-models.jar'
    ...
    def run(self, use_backup, path, use_svm, cross_val = False, j = 0):
        ...
        if cross_val:
            if use_backup == True:
                print(self.remaining_pos_vector)
                outF= open(str(FLAGS.hardcoded_path) + "/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/cross_val_remainder_'+str(j)+'.txt', "w")
                with open(FLAGS.test_path, "r") as fd:
                    for i, line in enumerate(fd):
                        if i in self.remaining_pos_vector:
                            outF.write(line)
            ...
            if use_svm == True:
                print(self.remaining_pos_vector)
                outF= open(str(FLAGS.hardcoded_path) + "/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/svm/cross_val_remainder_'+str(j)+'.txt', "w")
                with open(FLAGS.test_svm_path, "r") as fd:
                    for i, line in enumerate(fd):
                        if i in self.remaining_pos_vector:
                            outF.write(line)
