...
def change_y_to_onehot(y):
    ...
    return np.asarray(onehot, dtype=object)
...
def load_inputs_twitter(input_file, word_id_file, sentence_len, type_='', is_r=True, target_len=10, encoding='utf8'):
    ...
    all_y = y
    #print('y before chang to onehot ', len(y))
    y = change_y_to_onehot(y)
    if type_ == 'TD':
        return np.asarray(x), np.asarray(sen_len), np.asarray(x_r), np.asarray(sen_len_r), np.asarray(y)
    elif type_ == 'TC':
        ...
        return np.asarray(x), np.asarray(sen_len), np.asarray(x_r), np.asarray(sen_len_r), \
               np.asarray(y), np.asarray(target_words), np.asarray(tar_len), \
                np.asarray(all_sent, dtype= object), \
                    np.asarray(all_target, dtype= object), \
                        np.asarray(all_y, dtype= object)
    elif type_ == 'IAN':
        return np.asarray(x), np.asarray(sen_len), np.asarray(target_words), np.asarray(tar_len), np.asarray(y)
    else:
...
def load_inputs_twitter_(input_file, word_id_file, sentence_len, type_='', is_r=True, target_len=10, encoding='utf8'):
    ...
    if type_ == 'TD':
        ...
        return np.asarray(x), np.asarray(sen_len), np.asarray(x_r), np.asarray(sen_len_r), np.asarray(y), np.asarray(target_words), np.asarray(tar_len)
    else:
        return np.asarray(x), np.asarray(sen_len), np.asarray(y)
